...
- **Workspace copy is a basic streaming transfer**: `enclave workspace cp` requires a running workspace and currently supports one host path and one `ws:/` workspace path. Transfers do not overwrite an existing destination entry. Compression, progress reporting, glob expansion, resumable transfers, and parallel directory transfer are not yet supported.
