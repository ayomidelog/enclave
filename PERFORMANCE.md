...
| Setup command cache | explicit opt-in | `enclave up --cache-setup` and `enclave restart --cache-setup` cache successful setup commands by Enclavefile digest and command index; default setup reapplication is unchanged |
| Host directory archive traversal | one validation/archive walk | Host-to-workspace directory transfers now validate regular files, directories, symlinks, and reject special files while feeding the controlled tar writer, avoiding the prior recursive validation pass and host `tar` process |
